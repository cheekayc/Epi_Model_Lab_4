...
The SIR model with demography provides a way to study endemic diseases, in particular what proportion of the population are susceptible or infected when the disease become endemic.
Do the following simulations and report your findings:

Write a basic SIR function.
```{r basic SIR function}
...
To compare with SEIR model, write an SIR function with cumulative incidence:
```{r}
SIR = function(t, state, parameters){
  with(as.list(c(state, parameters)),{
